...
<<<<<<< HEAD
            ...
            try:
                paper_info['动机'], paper_info['方法'], paper_info['结论'] = coze_pdf_trans(pdf_url)
            except Exception as e_coze:
                print(f'coze api token过期或余额不足，请更新！{e_coze}')
                paper_info['动机'] = 'coze api token过期或余额不足！'
                paper_info['方法'] = 'coze api token过期或余额不足！'
                paper_info['结论'] = 'coze api token过期或余额不足！'
        
        with open(f'paper_data/{paper_date}/{id}.json', 'w') as fw:
